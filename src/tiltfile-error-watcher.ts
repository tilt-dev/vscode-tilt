...
const tiltfileErrorJsonPath = "{.status.targets[?(@.name=='tiltfile:update')].state.terminated}{'\\n'}"
...
export class TiltfileErrorWatcher implements Disposable {
    ...
    start() {
        const p = spawn("tilt", ['get', `-ojsonpath=${tiltfileErrorJsonPath}`, '--watch', 'session', 'Tiltfile'])
        ...
        p.stdout.pipe(split2()).on("data", (data) => {
            if (data.length > 0) {
                try {
                    ...
                    console.log(`tilt session watch: error processing session json: ${error}`)
                }
            }
        });
        p.on("close", (code) => {
